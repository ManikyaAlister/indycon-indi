...
# define model name 
models <- c("group+indi-prior-consensus")
...
if (model == "group-prior"){
  ...
  output <- brm(post_adjusted ~ (1|participant) + pre_adjusted + consensus * claim_type, data = filtered_data)
}else if (model == "group+indi-prior-consensusXclaim") {
  output <- brm(post_adjusted ~ (1 + pre_adjusted + consensus * claim_type | participant) + pre_adjusted + consensus * claim_type, data = filtered_data)
} else if (model == "group+indi-prior-consensus") {
  output <- brm(post_adjusted ~ (1 + pre_adjusted + consensus | participant) + pre_adjusted + consensus * claim_type, data = filtered_data, iter = 10000)
}
