...
model_LOOICs %>%
  filter(model != "group-prior-consensusXclaim-certainty", excluded_condition == "dependent") %>%
  select(-is_follow_up, -excluded_condition) %>%
    mutate(model_rank = rank(all_looic),
...
```{r}
model_LOOICs %>%
  filter(model != "group-prior-consensusXclaim-certainty", excluded_condition == "contested") %>%
  select(-is_follow_up, -excluded_condition) %>%
    mutate(model_rank = rank(all_looic),
         all_looic = round(all_looic),
         all_se = round(all_se))
...
model_LOOICs %>%
  filter(model != "group-prior-consensusXclaimXsource", excluded_condition == "dependent") %>%
  select(-is_follow_up, -excluded_condition) %>%
    mutate(model_rank = rank(all_looic),
         all_looic = round(all_looic),
         all_se = round(all_se))
...
```{r}
model_LOOICs %>%
  filter(model != "group-prior-consensusXclaimXsource", excluded_condition == "contested") %>%
  select(-is_follow_up, -excluded_condition) %>%
    mutate(model_rank = rank(all_looic),
         all_looic = round(all_looic),
         all_se = round(all_se))
