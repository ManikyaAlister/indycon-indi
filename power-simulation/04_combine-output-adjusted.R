...
for (j in all_trials_per_cell){
  ...
  for (i in start:n){
    ...
    load(here(paste0("power-simulation/output/P",i,"-output-",n_trials,"-trials-simdata-adjusted.Rdata")))
    id <- participants[i]
...
save(all_output, file = here("power-simulation/output/all-output-adjusted.Rdata"))
