...
# rename so that it doesn't conflict with generating data in loaed output
gen_data <- generating_params
...
for (j in all_trials_per_cell){
  ...
  for (i in start:n){
    ...
    load(here(paste0("simulation/output/P",i,"-output-",n_trials,"-trials-simdata-post.Rdata")))
...
save(all_output, file = here("simulation/output/all-output-post.Rdata"))
